...
//Obtener el body de la tabla
  ...
  searchInput.addEventListener("keyup", (eventito)=>{
   //escuchar cuando el usuario presiona una tecla, obtener valor del input, buscar en todos los usuarios aquellos que tengan ese texto, pintar solo los usuarios que hayan coincidida
    const inputValue=eventito.target.value.toLowerCase();

    // const usuariosFiltrados=usersArray.filter((usuario)=>usuario.fullname.toLocaleLowerCase().includes(inputValue))
...
<tr class="table-body">
  ...
  function formatDate(fecha){
    const collator=new Intl.DateTimeFormat("es-Ar", { //nueva instancia (objeto)
      day:"2-digit",
      month: "2-digit",
      year:"numeric"
    })
